...
\begin{figure}[H]
    \centering
    \includegraphics[scale=1]{Image PDFs/SLM/SLM-Main.drawio.pdf}
...
% The Decoder block is an essential part and most sequence-to-sequence models (like Transformers) include it. Its task is to take an input sequence and previous outputs to produce a new sequence.
\begin{figure}[H]
    \centering
    \includegraphics[scale=1]{Image PDFs/SLM/SLM-Decoder.drawio.pdf}
...
\begin{enumerate}
    ...
    It helps the decoder focus on all the previous tokens it has created. Because tokens are "masked," only tokens that appeared earlier in the sequence are considered, so later tokens do not influence the ones that appear afterward.
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[scale=1]{Image PDFs/SLM/SLM-Query-Key-Value.drawio.pdf}
