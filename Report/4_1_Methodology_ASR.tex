...
\subsubsection{Word Error Rate (WER) & Character Error Rate}
...
Character Error Rate (CER) is a commonly used error metric to compare the Automatic Speech Recognition (ASR) system performance on the character level. It is employed to quantify the correctness of transcription through comparison of the hypothesis of the system with a ground-truth reference transcript in terms of characters rather than words.

\begin{equation}
\text{Error Rate (WER or CER)} = \frac{S + D + I}{N}
\end{equation}
...
\begin{itemize}
	...
	\item N = Total number of words/character in the reference
\end{itemize}
